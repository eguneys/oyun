...
object Chronometer {
  ...
  case class Lap[A](result: A, nanos: Long) {
    ...
    def logIfSlow(threshold: Int, logger: oyun.log.Logger)(msg: A => String) = {
      if (millis >= threshold) log(logger)(msg)
      else this
    }
    def log(logger: oyun.log.Logger)(msg: A => String) = {
  ...
  case class FuLap[A](lap: Fu[Lap[A]]) extends AnyVal {
    ...
    // def mon(path: oyun.mon.TimerPath) = {
    //   lap dforeach { l =>
    //     path(oyun.mon).record(l.nanos)
    //   }
    //   this
    // }
  ...
  // def syncMon[A](path: oyun.mon.TimerPath)(f: => A): A = {
  //   val timer = path(oyun.mon).start
  //   val res   = f
  //   timer.stop()
  //   res
  // }
