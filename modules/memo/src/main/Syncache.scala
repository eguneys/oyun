...
final private[memo] class Syncache[K, V](
  ...
  private def waitForResult(k: K, fu: Fu[V], duration: FiniteDuration): V =
    try {
      //oyun.common.Chronometer.syncMon(_ => recWait) {
  ...
  // private val incMiss    = oyun.mon.syncache.miss(name).increment _
  // private val incTimeout = oyun.mon.syncache.timeout(name).increment _
  // private val recWait    = oyun.mon.syncache.wait(name)
  // private val recCompute = oyun.mon.syncache.compute(name)
}
