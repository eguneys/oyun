...
oyunkeyf.StrongSocket = function(url, version, settings) {
  ...
  const baseUrls = (
    d => [d].concat((d.includes('oyunkeyf.org') ? [5, 6, 7, 8, 9] : []).map(port => d + ':' + (9020 + port)))
...
oyunkeyf.StrongSocket.defaults = {
  events: {
    ...
    //     oyunkeyf.parseFen($(this).data("fen", e.fen).data("lastmove", e.lm));
    //   });
    // },
    // challenges: function(d) {
    //   oyunkeyf.challengeApp.update(d);
    // },
    // notifications: function(d) {
    //   oyunkeyf.notifyApp.update(d, true);
    // }
  },
  params: {
    sri: oyunkeyf.sri
